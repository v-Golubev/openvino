...
ngraph::snippets::pass::TokenizeMHASnippets::TokenizeMHASnippets(bool enable_transpose_tokenization) {
    ...
    register_matcher(std::make_shared<ngraph::pattern::Matcher>(m_matmul0, matcher_name),
        ...
        auto output = matmul0->input_value(1);
        auto parent = output.get_node_shared_ptr();
        ...
        while (is_supported_intermediate_op(parent)) {
            ...
            output = parent->input_value(0);
            parent = output.get_node_shared_ptr();
        }
        ...
        if (enable_transpose_tokenization && !are_ops_after_matmul2) {
