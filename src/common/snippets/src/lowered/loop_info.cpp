...
size_t LoopInfo::get_dim_idx() const {
    ...
    auto is_processed = [](const LoopPort& p) {
        return p.is_processed();
    };
    ...
    auto equal_dim_idxes = [&](const LoopPort& p) {
        return !p.is_processed() || p.get_dim_idx() == dim_idx;
    };
...
std::vector<bool> LoopInfo::get_is_incremented() const {
    ...
    iterate_through_ports([&values](const LoopPort& port) {
...
void LoopInfo::replace_with_new_ports(const ExpressionPort& actual_port,
                                      const std::vector<ExpressionPort>& target_ports) {
    ...
    std::transform(target_loop_ports.begin(),
                   target_loop_ports.end(),
                   target_ports.begin(),
                   target_loop_ports.begin(),
...
std::vector<LoopPort> LoopInfo::clone_loop_ports(const ExpressionMap& expr_map,
...
std::shared_ptr<LoopInfo> UnifiedLoopInfo::clone_with_new_expr(const ExpressionMap& expr_map,
                                                               LoopInfoMap& loop_map) const {
...
bool UnifiedLoopInfo::is_dynamic() const {
    return LoopInfo::is_dynamic() ||
           std::any_of(m_input_port_descs.cbegin(),
                       m_input_port_descs.cend(),
                       [](const LoopPortDesc& shift) {
                           return shift.is_dynamic();
                       }) ||
           std::any_of(m_output_port_descs.cbegin(), m_output_port_descs.cend(), [](const LoopPortDesc& shift) {
               return shift.is_dynamic();
           });
}
...
std::vector<int64_t> UnifiedLoopInfo::get_ptr_increments() const {
    ...
    values.reserve(get_input_count() + get_output_count());
    iterate_through_descs([&values](const LoopPortDesc& shift) {
        values.push_back(shift.ptr_increment);
    });
    return values;
}
...
std::vector<int64_t> UnifiedLoopInfo::get_finalization_offsets() const {
    ...
    values.reserve(get_input_count() + get_output_count());
    iterate_through_descs([&values](const LoopPortDesc& shift) {
        values.push_back(shift.finalization_offset);
    });
    return values;
}
...
std::vector<int64_t> UnifiedLoopInfo::get_data_sizes() const {
    ...
    values.reserve(get_input_count() + get_output_count());
    iterate_through_descs([&values](const LoopPortDesc& shift) {
        values.push_back(shift.data_size);
    });
    return values;
}
...
void UnifiedLoopInfo::replace_with_new_ports(const LoopPort& actual_port, const std::vector<LoopPort>& target_ports) {
    ...
    // Instead of call `LoopInfo::replace_with_new_ports()`, we can explicitly remove LoopPort since we already found
    // port_it.
    port_it = ports.erase(port_it);
    ports.insert(port_it, target_ports.cbegin(), target_ports.cend());
...
void UnifiedLoopInfo::replace_with_new_ports(const ExpressionPort& actual_port,
                                             const std::vector<ExpressionPort>& target_ports) {
    const auto& actual_port_type = actual_port.get_type();
    validate_new_target_ports(target_ports, actual_port_type);
...
InnerSplittedUnifiedLoopInfo::InnerSplittedUnifiedLoopInfo(size_t increment,
                                                           const std::vector<LoopPort>& entries,
                                                           const std::vector<LoopPort>& exits,
                                                           const std::vector<LoopPortDesc>& in_descs,
                                                           const std::vector<LoopPortDesc>& out_descs,
...
std::shared_ptr<LoopInfo> InnerSplittedUnifiedLoopInfo::clone_with_new_expr(const ExpressionMap& expr_map,
                                                                            LoopInfoMap& loop_map) const {
    if (loop_map.count(this) == 0) {
...
void InnerSplittedUnifiedLoopInfo::apply(const std::function<void(const LoopInfoPtr&)>& func,
                                         LoopInfoSet& applied_loops) {
    if (applied_loops.count(this) == 0) {
...
ExpandedLoopInfo::ExpandedLoopInfo(size_t work_amount,
                                   size_t increment,
                                   const std::vector<LoopPort>& entries,
                                   const std::vector<LoopPort>& exits,
    ...
    : LoopInfo(work_amount, increment, entries, exits, is_parallel),
      ...
      m_evaluate_once(evaluate_once) {
    OPENVINO_ASSERT(m_unified_loop_info, "Failed to create ExpandedLoopInfo: unified loop info is nullptr!");
    sort_ports();
}

std::shared_ptr<LoopInfo> ExpandedLoopInfo::clone_with_new_expr(const ExpressionMap& expr_map,
                                                                LoopInfoMap& loop_map) const {
    if (loop_map.count(this) == 0) {
        auto cloned_unified_loop_info =
            ov::as_type_ptr<UnifiedLoopInfo>(m_unified_loop_info->clone_with_new_expr(expr_map, loop_map));
        const auto& new_input_ports = clone_loop_ports(expr_map, m_input_ports);
        const auto& new_output_ports = clone_loop_ports(expr_map, m_output_ports);
...
bool ExpandedLoopInfo::is_dynamic() const {
    return LoopInfo::is_dynamic() ||
           std::any_of(m_ptr_increments.cbegin(),
                       m_ptr_increments.cend(),
                       [](size_t v) {
                           return utils::is_dynamic_value(v);
                       }) ||
           std::any_of(m_finalization_offsets.cbegin(), m_finalization_offsets.cend(), [](size_t v) {
               return utils::is_dynamic_value(v);
           });
}
...
void ExpandedLoopInfo::update_ptr_increments(const std::vector<int64_t>& new_values) {
    OPENVINO_ASSERT(new_values.size() == m_ptr_increments.size(),
                    "Failed to update ptr_increments: incompatible counts");
...
void ExpandedLoopInfo::update_finalization_offsets(const std::vector<int64_t>& new_values) {
    OPENVINO_ASSERT(new_values.size() == m_finalization_offsets.size(),
                    "Failed to update finalization_offsets: incompatible counts");
...
void ExpandedLoopInfo::replace_with_new_ports(const ExpressionPort& actual_port,
                                              const std::vector<ExpressionPort>& target_ports) {
    OPENVINO_ASSERT(target_ports.size() == 1, "ExpandedLoopInfo supports replace one port with only one port!");
    LoopInfo::replace_with_new_ports(actual_port, target_ports);
    sort_ports();
...
namespace {
void order_subvector(const std::vector<size_t>& indexes,
                     std::vector<int64_t>& original_vector,
                     size_t count,
                     size_t offset) {
