...
void InsertLoops::insertion(LinearIR& linear_ir, const LoopManagerPtr& loop_manager, size_t loop_id) {
    ...
    const auto loop_begin = loop_info->is_parallel() ?
                                std::make_shared<op::LoopBeginParallel>() :
                                std::make_shared<op::LoopBegin>();
    const auto loop_end = loop_info->is_parallel() ?
                                std::make_shared<op::LoopEndParallel>(loop_begin, work_amount, work_amount_increment, is_incremented, ptr_increments,
                                                                      finalization_offsets, io_data_sizes, in_num, out_num, loop_id) :
                                std::make_shared<op::LoopEnd>(loop_begin, work_amount, work_amount_increment, is_incremented, ptr_increments,
                                                              finalization_offsets, io_data_sizes, in_num, out_num, loop_id);
