...
bool ValidateLoops::run(LinearIR& linear_ir) {
    ...
    auto add_ports_dims_to_unique_dims = [](const std::vector<LoopPort>& loop_ports, std::set<size_t>& unique_dims) {
        for (const auto& loop_port : loop_ports) {
            ...
            const auto& dim = shape[*(layout.rbegin() + loop_port.dim_idx)];
    ...
    for (const auto& pair : loops) {
        ...
        add_ports_dims_to_unique_dims(entry_points, unique_dimensions);
        add_ports_dims_to_unique_dims(exit_points, unique_dimensions);
