...
void init_expr_descriptors(const ov::snippets::lowered::ExpressionPtr& expr,
    ...
    for (size_t i = 0; i < n_inputs; ++i) {
        ...
        const auto desc = std::make_shared<PortDescriptor>(node->get_input_partial_shape(i), subtensor, layout);
    ...
    for (size_t i = 0; i < n_outputs; ++i) {
        ...
        const auto desc = std::make_shared<PortDescriptor>(node->get_output_partial_shape(i), subtensor, layout);
