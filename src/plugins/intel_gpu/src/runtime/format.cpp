...
static const std::map<format::type, format_traits> format_traits_map{
    ...
    FMT_TRAITS(os_is_yx_osa4_isa8_osv8_isv4,
               1,
               1,
               2,
               0,
               ...
               "oixy",
               {{0, 4}, {1, 8}, {0, 8}, {1, 4}},
               {{0, 4}, {1, 8}, {0, 8}, {1, 4}}),  // NOLINT
    FMT_TRAITS(os_is_zyx_osa4_isa8_osv8_isv4,
               1,
               1,
               3,
               0,
    ...
    FMT_TRAITS(os_is_yx_osv16_isv32, 1, 1, 2, 0, {0, 1, 2, 3}, "oiyx", "oixy", {{0, 16}, {1, 32}}, {{0, 16}, {1, 32}}),                                          // NOLINT
    ...
    FMT_TRAITS(g_os_is_zyx_isv8_osv16_isv2,
               1,
               1,
               3,
               1,
...
format format::adjust_to_rank(format fmt, size_t new_rank) {
    ...
    auto is_adjustable = [](const format& fmt) -> bool {
        return !format::is_weights_format(fmt) && !format::is_image_2d(fmt) && !format::is_winograd(fmt);
    };
    ...
    for (const auto& kv : format_traits_map) {
        ...
        bool same_dims_scheme = current_traits.batch_num == candidate_traits.batch_num && current_traits.group_num == candidate_traits.group_num &&
...
const std::vector<std::pair<size_t, int>> format::per_axis_block_size(format fmt) {
    ...
    for (const auto& block : fmt.block_sizes()) {
        auto it = std::find_if(sizes_for_dims.begin(), sizes_for_dims.end(), [&block](const std::pair<size_t, int>& ele) {
            return ele.first == block.first;
        });
